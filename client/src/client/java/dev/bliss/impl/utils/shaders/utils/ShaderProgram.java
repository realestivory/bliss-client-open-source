...
import dev.bliss.impl.utils.shaders.exception.ShaderCompilationFailException;
import dev.bliss.impl.utils.shaders.exception.ShaderLinkFailException;
...
public class ShaderProgram {
    ...
    public void setUniformFloatBuffer(String uniformName, FloatBuffer values) {
        int location = getUniformLocation(uniformName);
        if (location != -1) {
            getUniformLocation(location, values.remaining(), values);
        }
    }

    private void getUniformLocation(int location, int remaining, FloatBuffer values) {
    }
}